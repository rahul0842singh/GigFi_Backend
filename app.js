...
// Get all members in a chatroom (with their wallet addresses)
app.get('/api/chatroom/:chatroomId/members', authenticateToken, (req, res) => {
  ...
  db.query(query, [chatroomId], (err, results) => {
    if (err) {
      ...
      return res.status(500).json({ error: "Failed to fetch chatroom members." });
    }
...
app.delete('/api/chatrooms/:chatroomId/:currentUserId/remove/:userId', authenticateToken, (req, res) => {
  const { chatroomId, currentUserId, userId } = req.params;
...
app.get('/api/forum/messages', authenticateToken, (req, res) => {
  const query = `
   SELECT 
    ...
    fm.is_read,
    fm.user_id,
    wc.walletaddress AS sender_wallet_address
FROM 
    forum_messages fm
JOIN 
...
app.post('/api/forum/messages/:currentUserId', authenticateToken, (req, res) => {
  const { message } = req.body;
  const currentUserId = req.params.currentUserId;
  const insertQuery = `
